...
export async function connect(ctx: DashboardContextContent, callback: (network: string, message: {[k: string]: any}) => void) {
  ...
  ctx.client?.on("message", (topic, payload) => handleMqttMessage(ctx, callback, topic, payload))
}

export function handleMqttMessage(ctx: DashboardContextContent, callback: (network: string, message: {[k: string]: any}) => void, topic: string, rawPayload: Uint8Array) {
  // let decodedPayload: { [k: string]: any } | null = null;
  if (topic == PRIMARY_TOPIC) {
    callback('primary', parseMessage(rawPayload, 'primary', ctx.primary_proto_root!))
  }
  else if (topic == SECONDARY_TOPIC) {
    callback('secondary', parseMessage(rawPayload, 'secondary', ctx.secondary_proto_root!))
  }
}
